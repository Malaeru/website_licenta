	...
	if(isset($_GET['nr_comanda']))
		{
			...
			$sql_cal_birou = "SELECT nr_cal_birou1, nr_cal_birou2, nr_cal_birou3, nr_cal_birou4, nr_cal_birou5 FROM comenzi_cal_birou INNER JOIN comenzi ON comenzi_cal_birou.nr_com_cal_birou= comenzi.nr_com_cal_birou AND comenzi.nr_comanda='$nr_comanda' AND comenzi.status='respins' AND comenzi.pret_total IS NULL;";

			$sql_cal_perete = "SELECT nr_cal_perete1, nr_cal_perete2, nr_cal_perete3, nr_cal_perete4, nr_cal_perete5 FROM comenzi_cal_perete INNER JOIN comenzi ON comenzi_cal_perete.nr_com_cal_perete= comenzi.nr_com_cal_perete AND comenzi.nr_comanda='$nr_comanda' AND comenzi.status='respins' AND comenzi.pret_total IS NULL;";

			$sql_carte = "SELECT nr_carte1, nr_carte2, nr_carte3, nr_carte4, nr_carte5 FROM comenzi_carti INNER JOIN comenzi ON comenzi_carti.nr_com_carte= comenzi.nr_com_carte AND comenzi.nr_comanda='$nr_comanda' AND comenzi.status='respins' AND comenzi.pret_total IS NULL;";

			$sql_cart_vizita = "SELECT nr_cart_vizita1, nr_cart_vizita2, nr_cart_vizita3, nr_cart_vizita4, nr_cart_vizita5 FROM comenzi_carti_vizita INNER JOIN comenzi ON comenzi_carti_vizita.nr_com_cat_vizita= comenzi.nr_com_cat_vizita AND comenzi.nr_comanda='$nr_comanda' AND comenzi.status='respins' AND comenzi.pret_total IS NULL;";

			$sql_flyere = "SELECT nr_flyer1, nr_flyer2, nr_flyer3, nr_flyer4, nr_flyer5 FROM comenzi_flyere INNER JOIN comenzi ON comenzi_flyere.nr_com_flyer= comenzi.nr_com_flyer AND comenzi.nr_comanda='$nr_comanda' AND comenzi.status='respins' AND comenzi.pret_total IS NULL;";

			$sql_pliant= "SELECT nr_pliant1, nr_pliant2, nr_pliant3, nr_pliant4, nr_pliant5 FROM comenzi_pliante INNER JOIN comenzi ON comenzi_pliante.nr_com_pliant= comenzi.nr_com_pliant AND comenzi.nr_comanda='$nr_comanda' AND comenzi.status='respins' AND comenzi.pret_total IS NULL;";
					...
					if($comanda[$i][0]== 'carte')
						{
							...
							$s_carte= "SELECT titlu, format_carte, pg_carte, nr_pag, hartie_carte, prt_coperta, grosime_carton, laminare, clp_flaps, tiraj_carte FROM carti WHERE nr_carte='$nr_carte' AND pret_carte IS NULL;";
					...
					elseif($comanda[$i][0]=='calendar-perete')
						{
							...
							$s_cal_perete= "SELECT format_cal_perete, hartie_cal_perete, tiraj_cal_perete  FROM calendare_perete WHERE nr_cal_perete= '$nr_cal_perete' AND pret_cal_perete IS NULL;";
					...
					elseif($comanda[$i][0]=='flyer')
						{
							...
							$s_flyer= "SELECT format_flyer, print_flyer, hartie_flyer, tiraj_flyer  FROM flyere WHERE nr_flyer= '$nr_flyer' AND pret_flyer IS NULL;";
					...
					elseif($comanda[$i][0]=='calendar-birou')
						{
							...
							$s_cal_birou= "SELECT format_cal_birou, hartie_cal_birou, tiraj_cal_birou  FROM calendare_birou WHERE nr_cal_birou= '$nr_cal_birou' AND pret_cal_birou IS NULL;";
					...
					elseif($comanda[$i][0]=='pliant')
						{
							...
							$s_pliant= "SELECT format_pliant, hartie_pliant, tiraj_pliant  FROM pliante WHERE nr_pliant= '$nr_pliant' AND pret_pliant IS NULL;";
					...
					elseif($comanda[$i][0]=='carte-vizita')
						{
							...
							$s_cart_vizita= "SELECT carton_cart_vizita, colt_rotund, tiraj_cart_vizita  FROM carti_vizita WHERE nr_cart_vizita= '$nr_cart_vizita' AND pret_cart_vizita IS NULL;";
