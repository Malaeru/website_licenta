...
if($_SESSION['nivel']!=2)
	...
	$sql = "SELECT data FROM comenzi WHERE nr_comanda='$nr_comanda' AND status='acceptat' AND stare='nefinalizat';";
	...
	if(isset($_GET['nr_comanda']))
		{
			...
			$sql_cal_birou = "SELECT nr_cal_birou1, nr_cal_birou2, nr_cal_birou3, nr_cal_birou4, nr_cal_birou5 FROM comenzi_cal_birou INNER JOIN comenzi ON comenzi_cal_birou.nr_com_cal_birou= comenzi.nr_com_cal_birou AND comenzi.nr_comanda='$nr_comanda' AND comenzi.status='acceptat' AND comenzi.stare='nefinalizat';";

			$sql_cal_perete = "SELECT nr_cal_perete1, nr_cal_perete2, nr_cal_perete3, nr_cal_perete4, nr_cal_perete5 FROM comenzi_cal_perete INNER JOIN comenzi ON comenzi_cal_perete.nr_com_cal_perete= comenzi.nr_com_cal_perete AND comenzi.nr_comanda='$nr_comanda' AND comenzi.status='acceptat' AND comenzi.stare='nefinalizat';";

			$sql_carte = "SELECT nr_carte1, nr_carte2, nr_carte3, nr_carte4, nr_carte5 FROM comenzi_carti INNER JOIN comenzi ON comenzi_carti.nr_com_carte= comenzi.nr_com_carte AND comenzi.nr_comanda='$nr_comanda' AND comenzi.status='acceptat' AND comenzi.stare='nefinalizat';";

			$sql_cart_vizita = "SELECT nr_cart_vizita1, nr_cart_vizita2, nr_cart_vizita3, nr_cart_vizita4, nr_cart_vizita5 FROM comenzi_carti_vizita INNER JOIN comenzi ON comenzi_carti_vizita.nr_com_cat_vizita= comenzi.nr_com_cat_vizita AND comenzi.nr_comanda='$nr_comanda' AND comenzi.status='acceptat' AND comenzi.stare='nefinalizat';";

			$sql_flyere = "SELECT nr_flyer1, nr_flyer2, nr_flyer3, nr_flyer4, nr_flyer5 FROM comenzi_flyere INNER JOIN comenzi ON comenzi_flyere.nr_com_flyer= comenzi.nr_com_flyer AND comenzi.nr_comanda='$nr_comanda' AND comenzi.status='acceptat' AND comenzi.stare='nefinalizat';";

			$sql_pliant= "SELECT nr_pliant1, nr_pliant2, nr_pliant3, nr_pliant4, nr_pliant5 FROM comenzi_pliante INNER JOIN comenzi ON comenzi_pliante.nr_com_pliant= comenzi.nr_com_pliant AND comenzi.nr_comanda='$nr_comanda' AND comenzi.status='acceptat' AND comenzi.stare='nefinalizat';";
...
<div class="navbar">
	...
	<div><a href="Termeniconditi.html">Termeni si conditi</a></div>
	<div><a href="Contact.html">Contact</a></div>
</div>
<center>
<form method="POST">
	<input type="submit" name="sub" value="descarca" class="btn">
	<button name="stare" method="POST" class="btn">gata</button>
...
for($i=0; $i<count($comandaDetaliat); $i++)
		{
			...
			if($comandaDetaliat[$i][0]== 'carte')
				{
					echo '
					<table border="3px" width="500px">
					<tr>
					<td rowspan="4"><img src="src/carte.png" width="100" height="100"></td></tr>
					<tr><td>Titlu: '.$comandaDetaliat[$i][1].'</td>
					<td>Format: '.$comandaDetaliat[$i][2].'</td>
					<td>Pagini: '.$comandaDetaliat[$i][3].'</td>
					</tr>
					<tr><td>numar pagini: '.$comandaDetaliat[$i][4].'</td</tr
					<tr>
			...
			elseif($comandaDetaliat[$i][0]=='calendar-perete')
				{
					...
					<table border="3px" width="500px">
					<tr>
					<td rowspan="4" width="100px"><img src="src/calendar-perete.png" width="100" height="100"></td>
			...
			elseif($comandaDetaliat[$i][0]=='flyer')
				{
					...
					<td>Print: '.$comandaDetaliat[$i][2].'</td>
					</tr>
					<tr>
					<td>Hartie: '.$comandaDetaliat[$i][3].'</td>
					<td>Tiraj: '.$comandaDetaliat[$i][4].'</td>
			...
			elseif($comandaDetaliat[$i][0]=='calendar-birou')
				{
					...
					<table border="3px" width="500px">
					<tr>
					<td rowspan="4" width="100px"><img src="src/calendar-birou.png" width="100" height="100"></td>
					</tr>
					<tr>
			...
			elseif($comandaDetaliat[$i][0]=='pliant')
				{
					...
					<table border="3px" width="500px">
					<tr>
					<td rowspan="4" width="100px"><img src="src/pliant.png" width="100" height="100"></td>
					</tr>
					<tr>
					<td>Format: '.$comandaDetaliat[$i][1].'</td></tr>
			...
			elseif($comandaDetaliat[$i][0]=='carte-vizita')
				{
					...
					<table border="3px" width="500px">
					<tr>
					<td rowspan="4" width="100px"><img src="src/carte-vizita.png" width="100" height="100"></td>
					</tr>
					<tr>
...
for($i=0; $i<count($comanda); $i++)
			...
			for($j=1; $j<count($comanda[$i]); $j++)
				{
					if($comanda[$i][0]== 'carte')
						{
							$nr_carte= $comanda[$i][$j];
							$s_carte= "SELECT carte_up FROM carti WHERE nr_carte='$nr_carte';";
					...
					elseif($comanda[$i][0]=='calendar-perete')
						{
							...
							$s_cal_perete= "SELECT calendare_perete_up FROM calendare_perete WHERE nr_cal_perete= '$nr_cal_perete';";
							...
							if($rcal_perete>0)
								{
									while($r=mysqli_fetch_assoc($cal_perete))
										{
											$files_zip= array($r['calendare_perete_up']);
										}
								}
						}
					...
					elseif($comanda[$i][0]=='flyer')
						{
							...
							$s_flyer= "SELECT flyer_up FROM flyere WHERE nr_flyer= '$nr_flyer';";
					...
					elseif($comanda[$i][0]=='calendar-birou')
						{
							...
							$s_cal_birou= "SELECT calendare_birou_up FROM calendare_birou WHERE nr_cal_birou= '$nr_cal_birou';";
							...
							if($rcal_birou>0)
								{
									while($r=mysqli_fetch_assoc($cal_birou))
										{
											$files_zip= array($r['calendare_birou_up']);
										}
								}
						}
					...
					elseif($comanda[$i][0]=='pliant')
						{
							...
							$s_pliant= "SELECT pliant_up FROM pliante WHERE nr_pliant= '$nr_pliant';";
					...
					elseif($comanda[$i][0]=='carte-vizita')
						{
							...
							$s_cart_vizita= "SELECT cart_vizita_up FROM carti_vizita WHERE nr_cart_vizita= '$nr_cart_vizita';";
	...
	if($zip->open($zip_nume,ZipArchive::CREATE)===TRUE)
		{
			...
			header("content-disposition: attachament; filename=$zip_nume");
			readfile($zip_nume);
	...
	if(isset($_POST['stare']))
		{	
			$stare="UPDATE comenzi SET stare='finalizat' WHERE nr_comanda='$nr_comanda' AND status= 'acceptat';";
			mysqli_query($conect, $stare);
			header('Location:productie.php');
